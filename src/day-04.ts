...
type TrimRight<S extends string> = S extends ` ${infer IRest}`
  ? TrimLeft<IRest>
  : S;
type TrimLeft<S extends string> = S extends ` ${infer IRest}`
  ? TrimLeft<IRest>
  : S;
...
> = TInput extends `${string}: ${infer ICards} | ${infer IWinning}\n${infer IRest}`
  ? ParseInput<
      ...
      [
        ...TResult,
        { cards: ParseCards<Trim<ICards>>; winning: ParseCards<Trim<IWinning>> }
      ]
    >
  : TResult;
...
type ScoreUnion<U, TResult extends any[] = [], TLast = PopUnion<U>> = [
  U
] extends [never]
  ? TResult['length']
  : ScoreUnion<
      Exclude<U, TLast>,
      TResult extends [] ? [any] : [...TResult, ...TResult]
    >;
...
type SizeUnion<U, TResult extends any[] = [], TLast = PopUnion<U>> = [
  ...
  : SizeUnion<
      Exclude<U, TLast>,
      TResult extends [] ? [any] : [...TResult, any]
    >;
...
> = TRows extends [infer IRow extends Row, ...infer IRestRows extends Row[]]
  ? TDeferred extends [
      infer ICopies extends number,
      ...infer IRestDeferred extends number[]
    ]
    ? ReduceRows<
        IRestRows,
        Intersperse<
