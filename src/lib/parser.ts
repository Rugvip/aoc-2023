...
type SplitComma<S extends string> = S extends `${infer IHead},${infer IRest}`
  ...
  : [strings.Trim<S>];
type ParseNumber<S extends string> = S extends `${infer INumber extends number}` ? INumber : never;
type ParseNumbers<
  S extends string,
...
> = T extends [infer INode extends ParsedTemplateNode, ...infer ITRest extends ParsedTemplateNode[]]
  ? INode extends { type: 'literal'; value: infer IValue extends string }
    ...
    ? INode extends {
      ...
      ? {
          ...
          numbers: ApplyTemplate<ISRest, ITRest, objects.Set<TResult, IName, ParseNumbers<ISNext>>>;
        }[IKind]
      : never
    : never
  : TResult;
...
declare const testParser: test.Describe<
  ...
  >,
  test.Expect<Parse<'data: 1, 2, 3, 4, 5', 'data: [counts: numbers]'>, { counts: [1, 2, 3, 4, 5] }>,
  test.Expect<
    Parse<
      'Derp 1: 1, 2, 3\nDerp 2: 4 5    6\n',
      '[lines: lines]',
